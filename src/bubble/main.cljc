(ns bubble.main
  "Kotodama WASM entrypoint for the Bubble clean-room actor (L4) — Clojure port.
  ...
  facts (`bubble.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bubble")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bubble.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bubble-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bubble-compat/ready)
